...
# If behavior cloning terminate the script early
    ...
    if args.dril:
        ...
        print(f'*** ensemble policy structure ***')
        print(ensemble_policy)
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            for i, info in enumerate(infos):
                if 'episode' in info.keys():
                    ...
                    train_hist.append(info['episode']['r'])

                    episode_uncertainty_rewards.append(running_uncertainty_reward[i] / info['episode']['l'])
                    u_hist.append(running_uncertainty_reward[i] / info['episode']['l'])
        ...
        rl = copy.deepcopy(rollouts)
        data = [rl, ensemble_variance_hist, raw_reward_hist]
        torch.save(data, fpath)
        print(f'Saving rollout to {fpath}')
        ...
        if (j % args.save_interval == 0 or j == num_updates - 1):
            ...
            model_file_name = f'dril_{args.env_name}_policy_ntrajs={args.num_trajs}_seed={args.seed}_ecf={args.entropy_coef}_update={j}'
            torch.save([
                actor_critic,
        ...
        if (args.eval_interval is not None and len(episode_rewards) > 1 and j % args.eval_interval == 0):
            ...
            file_name = f'{algo}_{args.env_name}_policy_ntrajs={args.num_trajs}_seed={args.seed}_ecf={args.entropy_coef}_updates={j}'
